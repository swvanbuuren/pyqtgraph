...
class GLSurfacePlotItem(GLMeshItem):
    ...
    **Bases:** :class:`GLMeshItem <pyqtgraph.opengl.GLMeshItem>`
    ...
    def __init__(self, parentItem=None, **kwds):
        ...
        self._z = None
        self._color = None
        self._x_shape = None  # Track x dimensionality
        self._y_shape = None  # Track y dimensionality
    ...
    def setData(self, **kwds):
        ...
        **Arguments:**
        ...
        # Check if x,y dimensions changed (handle both 1D and 2D cases)
        if x is not None:
            x_shape = np.asarray(x).shape
            x_1d = x_shape == (len(x),)  # check if 1D
        ...
        if y is not None:
            ...
            y_1d = y_shape == (len(y),)  # check if 1D
        ...
        if newVertexes or x is not None:
            ...
            x_arr = np.asarray(x)
            if x_arr.ndim == 1:
                # 1D case: broadcast to grid
                self._vertexes[:, :, 0] = x_arr.reshape(len(x_arr), 1)
            elif x_arr.ndim == 2:
                # 2D case: use directly
